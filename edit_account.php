...
//Mise à jour Photo Profil
  ...
  if($photo->process_user($user_id)){
    $session->msg('s','Photo télécharger.');
    redirect('edit_account.php');
    } else{
      $session->msg('d',"Photo non télécharger");
      redirect('edit_account.php');
    }
  }
?>
<?php
 //Mise à jour Informations
  if(isset($_POST['update'])){
    $req_fields = array('nom','username' );
    validate_fields($req_fields);
    if(empty($errors)){
             $id = (int)$_SESSION['user_id'];
           $name = remove_junk($db->escape($_POST['nom']));
    ...
    $result = $db->query($sql);
          if($result && $db->affected_rows() === 1){
            ...
            $session->msg('d',' Échec Mise à jour!');
            redirect('edit_account.php', false);
          }
    } else {
      ...
      <div class="panel-body">
          <form method="post" action="edit_account.php?id=<?php echo (int)$user['id'];?>" class="clearfix">
            <div class="form-group">
                  ...
                  <input type="name" class="form-control" name="nom" value="<?php echo remove_junk(ucwords($user['name'])); ?>">
            </div>
            <div class="form-group">
